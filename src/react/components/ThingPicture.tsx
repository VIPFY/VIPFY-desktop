...
const calculateInitials = (
  ...
  switch (type) {
    ...
    case ThingType.Team:
      return Array.from(name)[0] || "?"; // Array.from handles "😀" correctly
...
const getSourceSet = (type: ThingType, picture: string, size: number): string => {
  ...
  switch (type) {
    case ThingType.User:
      return getSourceSetUser(picture, size);
    case ThingType.Team:
      return getSourceSetTeam(picture, size);
    case ThingType.Service:
      return getSourceSetApp(picture, size);
    case ThingType.Company:
      return getSourceSetTeam(picture, size);
  }
};
...
const getSrc = (type: ThingType, picture: string, size: number): string => {
  ...
  switch (type) {
    case ThingType.User:
      return getImageUrlUser(picture, size);
    case ThingType.Team:
      return getImageUrlTeam(picture, size);
    case ThingType.Service:
      return getImageUrlApp(picture, size);
    case ThingType.Company:
      return getImageUrlTeam(picture, size);
  }
};
...
const computeBackgroundStyle = (
  ...
  switch (type) {
    case ThingType.User:
      return { backgroundColor: design == ThingDesign.v1 ? "#5d76ff" : "#B2D0FB" };
    case ThingType.Team:
      return { backgroundColor: design == ThingDesign.v1 ? "#9C13BC" : "#E8A2D4" };
    case ThingType.Service:
      return { backgroundColor: design == ThingDesign.v1 ? "#5d76ff" : "#B2D0FB" };
    case ThingType.Company:
      return { backgroundColor: design == ThingDesign.v1 ? "#9C13BC" : "#E8A2D4" };
...
const computeImgProps = (
  ...
  return {
    ...
    style: {
      width: "100%",
      height: "100%",
      objectFit: picture.startsWith("data:") ? "cover" : "contain",
      ...additionalStyle
    }
  };
};
...
const ThingPictureRaw: React.FC<ThingProps> = React.memo(
  ({
    ...
    const additionalStyle = computeBackgroundStyle(state, type, design, picture, color);
    initials = initials || calculateInitials(state, type, name);
    const borderRadius = size <= 32 ? "3px" : size >= 128 ? "0.5em" : "4px";
    const imageProps = computeImgProps(state, type, picture, size);

    return (
      <div
        {...propsModifier({
          title: hideTitle ? null : name,
          className: classNames(className || "managerSquare", {
        ...
        {(!picture || state != ThingState.Normal) && (
          <span
            style={{
              width: finalSize,
              height: finalSize,
...
const ThingPictureLoadState: React.FC<ThingProps> = props => {
  ...
  if (isError) {
    return (
      <ThingPictureRaw {...props} picture={null} onError={setError} onLoad={setIsDoneLoading} />
    );
  }

  if (isDoneLoading || !props.picture || props.picture.startsWith("data:")) {
    return <ThingPictureRaw {...props} onError={setError} onLoad={setIsDoneLoading} />;
  }
...
const ThingPictureEditable: React.FC<ThingProps> = props => {
  ...
  React.useEffect(() => {
    const effectFunction = async picture => {
      if (picture instanceof Blob) {
        setPicture(
          "data:image/png;base64," + Buffer.from(await picture.arrayBuffer()).toString("base64")
        );
      } else {
        setPicture(picture);
      }
  ...
  const editable =
    ...
    (props.onEdit ||
      (!meLoading &&
        ...
        meData &&
        meData.me &&
        (!!meData.me.isadmin || meData.me.id == props.id)));
  ...
  return (
    <ThingPictureLoadState
      ...
      {...propOverwrites}
      propsModifier={
        props.propsModifier ? s => getRootProps(props.propsModifier(s)) : getRootProps
      }>
      <input {...getInputProps()} />
...
export const ThingPicture: React.FC<ThingProps> = props => {
  // this component is purely a performance optimization, bypassing unnecessary logic
