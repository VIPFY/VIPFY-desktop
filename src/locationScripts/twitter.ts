...
function onLoad() {
  if (document.querySelector("input[name='session[password]']")) {
    ...
    ipcRenderer.on("loginData", (e, key) => {
      ...
      document.querySelector("input[name='session[username_or_email]']")!.value = username;

      document
        .querySelector("input[name='session[username_or_email]']")
      ...
      document.querySelector("input[name='session[password]']")!.value = password;

      document
        .querySelector("input[name='session[password]']")
        .dispatchEvent(new Event("change", { bubbles: true, cancelable: true }));
  ...
  } else if (document.getElementById("page-container")) {
    hideLoading();
  } else {
